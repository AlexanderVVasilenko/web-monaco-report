...
def read_and_create_race(file_path: str) -> dict:
    ...
    return Race.create(year=year, location=location, race_name=name)


def read_lap_time_and_create_races(start_log_path: str, end_log_path: str, abbrs_path: str, race: Race) -> None:
    racers = parse_logs(start_log_path, end_log_path, load_abbreviations(abbrs_path))
    racers = racers[0] + racers[1]
    for racer in racers:
        LapTime.create(race=race, racer=Racer.get(Racer.driver_id == racer.driver_id), lap_time=racer.lap_time)
...
if __name__ == "__main__":
    ...
    read_lap_time_and_create_races("inputs/start.log", "inputs/end.log", "inputs/abbreviations.txt", race_obj)
